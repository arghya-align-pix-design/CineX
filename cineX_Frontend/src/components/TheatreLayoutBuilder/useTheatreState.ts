...
import { mkSeat, mkRow, mkSection, createSeedLayout, SEAT_TYPES } from './constants'
...
export function useTheatreState() {
  ...
  const enterPreview = useCallback(() => {
    ...
    // Randomly book ~27% of non-removed seats
  ...
  const addSection = useCallback(() => {
    saveHistory()
    const used = new Set(layout.filter((i): i is SectionItem => i.kind === 'section').map(s => s.seatType))
  ...
  const addRow = useCallback((sectionId?: number) => {
    ...
    setLayout(l => {
      if (sectionId !== undefined) {
        ...
        // Find last row belonging to this section
        let insertIdx = secIdx + 1
        while (insertIdx < l.length && l[insertIdx].kind === 'row') insertIdx++
        const newRow = mkRow(Array.from({ length: n }, () => mkSeat(sec.seatType)))
        return [...l.slice(0, insertIdx), newRow, ...l.slice(insertIdx)]
      }
      // Append to end, infer type from last section
      ...
      return [...l, mkRow(Array.from({ length: n }, () => mkSeat(type)))]
    })
  ...
  const copyLastRow = useCallback(() => {
    saveHistory()
    setLayout(l => {
      const lastRowIdx = l.map((item, i) => item.kind === 'row' ? i : -1).filter(i => i >= 0).pop()
      if (lastRowIdx === undefined) return l
      const src = l[lastRowIdx] as RowItem
      const newRow = mkRow(src.seats.map(s => ({ ...mkSeat(s.type), removed: s.removed })))
  ...
  const addColumn = useCallback(() => {
    ...
    setLayout(l => l.map(item => {
      ...
      return { ...item, seats: [...item.seats, mkSeat(firstType)] }
    }))
  ...
  const removeColumn = useCallback(() => {
    ...
    setLayout(l => l.map(item => {
      ...
      const removed = item.seats[item.seats.length - 1]
      setSelected(prev => { const n = new Set(prev); n.delete(removed.id); return n })
  ...
  // ── Stats ──
  const stats = (() => {
    let rows = 0, total = 0, silver = 0, gold = 0, exec = 0, recl = 0
    for (const item of layout) {
      if (item.kind === 'row') {
        rows++
        for (const s of item.seats) {
    ...
    return { rows, total, silver, gold, exec, recl }
  })()
  ...
  const pvTotal = (() => {
    ...
    let currentPrice = 0
    for (const item of layout) {
      if (item.kind === 'section') currentPrice = item.price
      if (item.kind === 'row') {
        for (const s of item.seats) {
          if (pvSelected.has(s.id) && !pvBooked.has(s.id) && !s.removed) total += currentPrice
        }
      }
    }
    return total
